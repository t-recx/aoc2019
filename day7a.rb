...
def execute(program, phase_setting, input)
  ...
  loop do
    ...
    if [1, 2, 7, 8].include? opcode
      ...
      if opcode == 3
        program[position] = phase_setting || input

        phase_setting = nil
      end

      return program[position] if opcode == 4
...
(0..4).to_a.permutation.each do |p|
  ...
  p.each do |phase_setting|
    input = execute(program.clone, phase_setting, input)
  end
